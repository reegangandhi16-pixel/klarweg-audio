/**
 * scripts/generate-a2-audio.ts
 * ----------------------------------------------------------------------------
 * Dual-voice A2 vocabulary audio generator (female + male).
 *
 *   • Source: content/klarweg-a2-all-german-words.txt  (articles preserved: der Tisch, die Lampe…)
 ...
 *   • Manifest: public/audio/a2-words-manifest.json
 ...
 *
 * Commands:
 *   npm run generate-a2-audio:dry     preview counts, no API calls
 *   npm run generate-a2-audio         generate missing female+male MP3s
 *   npm run generate-a2-audio:stats   report coverage from disk + manifest
 */
import { createHash } from 'node:crypto';
import { mkdirSync, existsSync, writeFileSync, readFileSync } from 'node:fs';
import { resolve } from 'node:path';
import { config as loadEnv } from 'dotenv';
import { parseA2Words } from './lib/parse-a2-words.ts';
...
const MANIFEST = resolve(ROOT, 'public/audio/a2-words-manifest.json');
...
async function main() {
  const words = parseA2Words();
  console.log('\n🔤  Klarweg A2 vocabulary audio (dual voice)');
  console.log(`    source: content/klarweg-a2-all-german-words.txt`);
  console.log(`    female: ${VOICE_FEMALE}   male: ${VOICE_MALE}`);
  console.log(`    A2 entries detected: ${words.length}`);
  ...
  console.log(`\n📦 a2-words-manifest.json: ${Object.keys(manifest).length} entries`);
